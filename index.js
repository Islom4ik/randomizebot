...
bot.command("random", async (ctx) => {
    if(await ctx.message.from.id == '5103314362') {
        try {
            await ctx.replyWithHTML(`👔 Следующими дежурными будут => <b>${users[random(0, 15)]}</b> и <b>${users[random(0, 15)]}</b>`, Markup.inlineKeyboard(
                [
                    [Markup.button.callback('REROLL ONE', 'rro'), Markup.button.callback('REROLL TWO', 'rrt')]
                ]
            ))
        }catch(e) {
            console.error();
...
bot.command("singleRandom", async (ctx) => {
    if(await ctx.message.from.id == '5103314362') {
        try {
            await ctx.replyWithHTML(`👔 Следующим дежурным будет => <b>${users[random(0, 15)]}</b>`, Markup.inlineKeyboard(
                [
                    [Markup.button.callback('REROLL ONE', 'rro'), Markup.button.callback('REROLL TWO', 'rrt')]
                ]
            ))
...
bot.action('rro', async ctx => {
    try {
        if(await ctx.callbackQuery.from.id == '5103314362') {
            await ctx.answerCbQuery(`Обработка...`, {show_alert: false});
            await ctx.replyWithHTML(`👔 Следующим дежурным будет => <b>${users[random(0, 15)]}</b>`);
        }else if(await ctx.callbackQuery.from.id == '1050880283') {
            await ctx.answerCbQuery(`Обработка...`, {show_alert: false});
            await ctx.replyWithHTML(`👔 Следующим дежурным будет => <b>${users[random(0, 15)]}</b>`);
        }else {
            await ctx.answerCbQuery('Это вас не касается!')
        }
    }catch(e) {
        console.error(e);
    }
...
bot.action('rrt', async ctx => {
    try {
        if(await ctx.callbackQuery.from.id == '5103314362') {
            await ctx.answerCbQuery(`Обработка...`, {show_alert: false});
            await ctx.replyWithHTML(`👔 Следующими дежурными будут => <b>${users[random(0, 15)]}</b> и <b>${users[random(0, 15)]}</b>`);
        }else if(await ctx.callbackQuery.from.id == '1050880283') {
            await ctx.answerCbQuery(`Обработка...`, {show_alert: false});
            await ctx.replyWithHTML(`👔 Следующими дежурными будут => <b>${users[random(0, 15)]}</b> и <b>${users[random(0, 15)]}</b>`);
        }else {
